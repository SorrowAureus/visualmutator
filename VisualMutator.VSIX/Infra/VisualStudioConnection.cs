...
namespace PiotrTrzpil.VisualMutator_VSPackage.Model {
  ...
  public class VisualStudioConnection : IHostEnviromentConnection {
    ...
    public IEnumerable<DirectoryPathAbsolute> GetProjectPaths() {
      return _dte.Solution.Cast<Project>()
        .Select(project => new { project, confManager = project.ConfigurationManager })
        .Where(@t =>
          @t.confManager != null && @t.confManager.ActiveConfiguration != null &&
          @t.confManager.ActiveConfiguration.IsBuildable)
        .Select(@t => new { @t, values = @t.project.Properties.Cast<Property>().ToDictionary(prop => prop.Name) })
        .Where(@t => @t.values.ContainsKey("LocalPath"))
        .Select<dynamic, DirectoryPathAbsolute>(@t => @t.values["LocalPath"].Value.CastTo<string>().ToDirPathAbs());
    }
    ...
    public IEnumerable<FilePathAbsolute> GetProjectAssemblyPaths() {
      ...
      return listt.Where(project => project.ConfigurationManager != null)
        .Select(project => new { project, config = project.ConfigurationManager.ActiveConfiguration })
        .Where(@t => @t.config != null && @t.config.IsBuildable)
        .Select(@t => new { @t, values = @t.project.Properties.Cast<Property>().ToDictionary(p => p.Name) })
        .Where(@t => @t.values.ContainsKey("LocalPath") && @t.values.ContainsKey("OutputFileName"))
        .Where(@t => @t.@t.config.Properties.Cast<Property>().Any(p => p.Name == "OutputPath"))
        .Select(@t => new { @t, localPath = @t.values["LocalPath"].Value.CastTo<string>() })
        .Select(@t => new { @t, outputFileName = @t.@t.values["OutputFileName"].Value.CastTo<string>() })
        .Select(@t => new {
          @t,
          outputDir = (string) @t.@t.@t.@t.config.Properties.Cast<Property>()
            .Single(p => p.Name == "OutputPath")
            .Value
        })
        .Select(@t => Path.Combine((string) @t.@t.@t.localPath, @t.outputDir, (string) @t.@t.outputFileName)
          .ToFilePathAbs());
    }
